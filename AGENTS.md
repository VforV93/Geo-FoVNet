...
5. **Half-precision storage**: preprocessing compresses graphs to fp16 **by default** (`--no_compress` opts out); everything is upcast to fp32 at load. Measured impact is negligible: diffing stored fp16 graphs against fp32 recomputes from the same STEP files (30 models across all 4 datasets, smoke samples) gave a worst-case relative error of 0.04% (abs 3.9e-3), with the smallest observed face area at 0.34 — six orders of magnitude above fp16's subnormal range. The only theoretical failure is a sliver face with area < ~3e-8 (after unit-box scaling), which fp16 flushes to exactly 0; the sanity check in `geometry_features.py` allows areas down to ~1.2e-11, so `_area()` prints a warning if such a face is ever encountered. Distances can't hit this: ray tolerance (1e-4) / mesh-caster epsilon (≥1e-3) bound them well above fp16's floor, and coordinates/normals/dots are O(1) signed values where near-zero flushing is harmless.
